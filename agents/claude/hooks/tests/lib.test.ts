...
describe('turnText', () => {
  test('returns only assistant text after the last user entry', () => {
    const entries = [user('q1'), assistant('old'), user('q2'), assistant('a'), assistant('b')]
    expect(turnText(entries)).toBe('a\nb')
  })

  test('tool_result user entries bound the turn too', () => {
    const entries = [user('q'), assistant('before'), toolResultUser(), assistant('after')]
...
describe('lastUserText', () => {
  test('skips tool_result carriers and returns the last human prompt', () => {
    const entries = [user('first'), assistant('a'), user('second'), toolResultUser()]
