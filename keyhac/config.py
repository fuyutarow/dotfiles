...
def configure(keymap):
    ...
    def recenter():
        if checkWindow("sakura.exe$",
                       "EditorClient$|SakuraView166$"):  # Sakura Editor
    ...
    def kill_line(repeat=1):
        ...
        if repeat == 1:
            ...
            if checkWindow("cmd.exe$|powershell.exe$",
                           "ConsoleWindowClass$"):  # Cmd or PowerShell
                kill_region()

            elif checkWindow("Hidemaru.exe$",
    ...
    def mark_whole_buffer():
        ...
        elif checkWindow("powershell.exe$",
    ...
    def query_replace():
        if (checkWindow("sakura.exe$", "EditorClient$|SakuraView166$")
                or  # Sakura Editor
    ...
    def keyboard_quit():
        ...
        if not (checkWindow("EXCEL.EXE$", "EXCEL")
                or checkWindow("Evernote.exe$", "WebViewHost$")):
    ...
    def checkWindow(processName, className):
        return ((processName is None
                 or re.match(processName,
                             keymap.getWindow().getProcessName()))
                and (className is None
                     or re.match(className,
                                 keymap.getWindow().getClassName())))
    ...
    def restore_window():
        window_list = getWindowList()
        if not (sys.getwindowsversion().major == 6
