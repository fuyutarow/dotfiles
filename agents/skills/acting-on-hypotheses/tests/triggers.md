# Fire / no-fire desk-check — acting on hypotheses

Read only the name and description. Any unresolved race is a description or boundary defect.

## FIRES

| Ask | Why here |
|---|---|
| “This selected transfer candidate would gate an irreversible migration. Design the cheapest target-side kill test.” | one selected expensive forward bet; donor success is insufficient |
| 「転用仮説の対応関係を、対象側でどう反証すればよいか決めたい」 | target-side discriminating test and threshold |
| “The target result is still UNTESTED; precommit PASS/FAIL outcomes before we build on it.” | Loop owns the outcome table and threshold |
| “The target-side test failed; record its observation, prewritten threshold, and locus before we decide what to do.” | Loop returns a `TARGET RESULT` for the selected expensive tree; it does not classify the map |
| “We have a selected hypothesis tree whose weakest node decides a costly commitment.” | hard gate for one selected tree |
| 「この賭けを続行・ピボット・撤退する条件を、検証前に書く」 | WIN/KILL/LOSS and commitment boundary |
...
The description and core must retain one selected expensive/irreversible tree, the prewritten
threshold and outcome table, and the target-side-evidence guard. They must not imply that a donor
result, a surface analogy, or agent agreement can move target confidence. They must make `TARGET RESULT`
the only AOH transfer output and reserve `MAPPING-BREAK` emission/reclassification for
