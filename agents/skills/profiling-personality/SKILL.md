...
| `references/conversational-metrics.md` | countable L1 features for chat corpora — topic-initiation share & territory (other-oriented-at-self-cost), transition type, question rate, unsolicited specifics, latency baselines, style convergence, relational-move events; denominators + state-vs-trait discipline | ANY dialogue-log profiling; run the counts BEFORE impressions (Layer 1) |
