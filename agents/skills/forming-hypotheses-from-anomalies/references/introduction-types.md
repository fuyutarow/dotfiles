...
> **Scope**: step 7 of SKILL.md's procedure. The `Introduction type` row is not a label. Each type
...
|---|---|---|---|
| `NONE` | the closed route explained it | `Closed route: SUCCEEDED` and `Status: CLOSED-VOCABULARY`, and A4 is still owed; the packet is terminal, no handoff | claiming NONE while the explanation quietly uses a term you never supplied; run the introduced-terms check |
...
shadow: words in the `Hypothesis:` row absent from what you cited. Treat it as a REPORT. The same undecidability that governs A3 governs this (VOC-003). A word you
