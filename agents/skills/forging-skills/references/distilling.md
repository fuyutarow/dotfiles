...
| **SURVEY / corpus** (many cases or papers) | Do NOT distill from the raw corpus. Run it as an **adversarially-verified fleet FIRST** — corpus synthesis (claim ledger, reconciliation, GRADE) is owned by systematizing-knowledge — then distill THE RESULT (the reconciled ledger), not the papers | growing-oss-adoption: 90-agent adversarial survey → reconciled mechanisms → skill |
...
2. **Resolution on load-bearing claims.** Any claim a rule leans on states mechanism + magnitude +
   ...
   resolution grammar, not restated here. "X improves quality" is not yet distilled; "X cuts
...
| auditing-audience-facing-prose | human blindness to LLM-ish prose | the audit skill ITSELF over-flags at machine speed | "name the slots, never the sins" anti-priming |
