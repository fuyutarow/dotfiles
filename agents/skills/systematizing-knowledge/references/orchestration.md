...
| 10 write: audit | **FAN-OUT** | adversarial verification per load-bearing claim; resolution audit per section (`resolution.md` §3–4); prose audit (the `linting-prose` lens) — all read-only (§4). Findings loop back to step 3 |
