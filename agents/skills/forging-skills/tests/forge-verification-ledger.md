...
| MAJOR | self-contradiction | `scripts/` role stated wider than greppable floors — racing the floor-vs-semantic boundary the skill itself teaches | Narrowed to floors; `skill-check.sh` header declares NOT-semantic; F1 pointer re-aimed at `references/architecture.md` §5 |
