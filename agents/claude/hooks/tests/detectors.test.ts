...
describe("detect-prose-correo", () => {
  ...
  const transcript = () =>
    writeTranscript([user("説明して"), assistant("本文です。")]);
  ...
  test("coinage hit -> exit 2; coinage exit 2 (no dict) -> FAIL-SAFE pass", () => {
    expect(
      runHook(HOOK, stopPayload(transcript()), withFake("coinage")).code,
    ).toBe(2);
    expect(
      runHook(HOOK, stopPayload(transcript()), withFake("nodict")).code,
    ).toBe(0);
  });

  test("clean text -> exit 0", () => {
    expect(
      runHook(HOOK, stopPayload(transcript()), withFake("clean")).code,
    ).toBe(0);
  });

  const realCorreoInFallbacks =
    existsSync("/usr/local/bin/correo") ||
    existsSync("/home/linuxbrew/.linuxbrew/bin/correo");
  test.skipIf(realCorreoInFallbacks)(
    "correo absent -> FAIL-SAFE exit 0",
    () => {
