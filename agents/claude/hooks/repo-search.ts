...
function exactlyOneQuery(route: Route, queries: string[]): string {
...
function rgFlags(values: {
  ...
  if (values.context !== undefined) {
    flags.push("--context", String(positiveInteger("context", values.context)));
  }
...
async function main(): Promise<number> {
  const parsed = parseArgs({
    ...
    options: {
      ...
      path: { type: "string", multiple: true, short: "p" },
      glob: { type: "string", multiple: true, short: "g" },
      limit: { type: "string", default: "8" },
      "timeout-ms": { type: "string", default: "120000" },
      "ignore-case": { type: "boolean" },
      hidden: { type: "boolean" },
      context: { type: "string" },
      "files-with-matches": { type: "boolean" },
      count: { type: "boolean" },
  ...
  const [rawRoute, ...unexpectedPositionals] = parsed.positionals;
  if (!isRoute(rawRoute)) throw new Error(`unknown route '${rawRoute ?? ""}'`);
  ...
  const queries = parsed.values.query ?? [];
  const paths = parsed.values.path ?? [];
  const timeoutMs = positiveInteger("timeout-ms", parsed.values["timeout-ms"]);
  const limit = positiveInteger("limit", parsed.values.limit);
  ...
  if (rawRoute === "concept" || rawRoute === "battery") {
    if (rawRoute === "concept") exactlyOneQuery(rawRoute, queries);
    if (rawRoute === "battery") {
    ...
    return runCccSearch(rawRoute, queries, paths[0], limit, timeoutMs);
  }
  ...
  if (rawRoute === "files") {
    if (queries.length > 0) throw new Error("files does not accept --query");
    return runRg(
      rawRoute,
      undefined,
