...
half-done rebases. Every incident in §3 disappears under this layout.
...
| `git worktree remove <path>` when done; `git worktree prune` after a deleted directory | a stale worktree entry keeps its branch un-switchable |
...
| S5 | **No `gc`, `prune`, `reflog expire`, `repack` while any peer may write** — `git maintenance run` is the only background housekeeping allowed | `pgrep -af 'git (gc\|repack\|prune)'` empty |
...
Every row would have been avoided by §1. The protocol in §2 exists for the repos that cannot.
