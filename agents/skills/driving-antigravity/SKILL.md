...
| headless `agy -p` produces EMPTY stdout; stderr: a tool required the "command" permission that headless mode cannot prompt for → auto-denied (observed 2026-07-24; `--sandbox --mode plan` does NOT lift it; the host harness may block `--dangerously-skip-permissions`) | headless runs cannot grant agy's shell-tool permission; settings.json allow-rules are the sanctioned fix but the file location is non-obvious | for READ-ONLY audit/derivation: bypass tools entirely — INLINE the evidence into `-p` (exact code/doc excerpts + summarized JSON) and instruct "judge only within the enclosed evidence"; disclose the scope limit in the record. Always snapshot `git status --porcelain` before/after and diff to prove no writes (UNCONFINED defense). Verified pattern: 3 audits + 2 theory drafts landed this way, 2026-07-24 |
