...
**RELAY**, **repo-retrieve**, **fire / no-fire**, **ENUMERATE** (a fleet-wide operation starts
...
> call-site enumeration, and structural patterns belong to `repo-retrieve`'s rg / Serena /
...
| **CC3 ROUTE** | Name the query shape before choosing the engine (the QUERY-SHAPE ladder — §Routing). On house hosts, invoke `repo-retrieve`; its route is the declaration. Wrong shape (e.g. semantic top-k for "every call site of X") is a gate violation even if results look plausible — the live trial measured both missed call sites AND false positives on exactly that shape (numbers → `references/catalog.md`) | the `repo-retrieve` route + exact query |
