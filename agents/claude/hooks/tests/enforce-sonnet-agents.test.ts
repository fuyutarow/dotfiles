...
const pre = (tool_name: string, tool_input: unknown) => ({ tool_name, tool_input })
...
describe('Agent / Task', () => {
  ...
  test('sonnet model -> silent pass', () => {
    const r = runHook(HOOK, pre('Agent', { prompt: 'x', model: 'claude-sonnet-5' }))
    expect(r.code).toBe(0)
    expect(r.stdout.trim()).toBe('')
  })

  test('fork inherits the parent model -> silent pass even with model set', () => {
    const r = runHook(HOOK, pre('Agent', { subagent_type: 'fork', model: 'opus' }))
    expect(r.code).toBe(0)
    expect(r.stdout.trim()).toBe('')
  })
})
...
describe('Workflow', () => {
  ...
  test("model:'sonnet' inside a prompt STRING cannot fake a pass", () => {
    const r = runHook(HOOK, wf(`await agent("use model:'sonnet' please", {schema: S})`))
    expect(decisionOf(r.stdout).permissionDecision).toBe('deny')
  })
  ...
  test('unreadable scriptPath -> ask', () => {
    const r = runHook(HOOK, pre('Workflow', { scriptPath: '/nonexistent/wf.js' }))
    expect(decisionOf(r.stdout).permissionDecision).toBe('ask')
  })
})
