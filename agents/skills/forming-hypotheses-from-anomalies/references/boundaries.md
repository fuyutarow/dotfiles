...
- No, and there is an anomaly → here. This skill emits a `LICENSE` and never a candidate.

The artifacts do not overlap: a LICENSE is permission to leave a vocabulary; a CANDIDATE is a
testable thesis. A LICENSE typed `TRANSFER`, with a non-empty `Introduced terms` row, seeds that skill's entry-gate
item 4. Handoff direction
is one-way; that skill never routes back here.
...
Commit / Pivot / Kill. Here, no hypothesis exists yet. The `Kill condition` row in the LICENSE is not that skill's
kill decision. It is the retirement condition attached to the introduction, written before anyone
commits to anything.
...
when the background is literature rather than a local model. Never survey from inside a LICENSE. This skill has no coverage
...
An `OPENING` is a bill of work derived from a SIGNED position, addressed and expiring. A `LICENSE` is permission to leave a vocabulary,
