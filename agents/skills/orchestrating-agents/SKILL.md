...
description: >-
  ...
  visibility、dependency、veto、verification、acceptance、GPU/CPU/RAM/VRAM resource admissionで使う。
  比較土俵/基準線/漏れ/退行の証拠判断→validating-experimental-evidence。裸の「研究を進めて」では発火せず、
...
P8/P9 の番号は再利用しない。比較条件、交絡、漏れ、退行、証拠の採否は
`validating-experimental-evidence` に移した。本skillはpolysearchの正本findingにある
`EVIDENCE DISPOSITION` のlocus/digestをdispatch、visibility、acceptanceの入力として受ける。
...
計算資源と保存は本skillのreferenceを読む。測定、交絡、機構なし対照の意味は
`validating-experimental-evidence` が判定した正本findingのlocus/digestを受ける。
...
| 複数armの実長、基準線、漏れ、交絡、退行を判定する。 | `NO-FIRE / CO-FIRE` | 証拠の意味は`validating-experimental-evidence`。署名済みmapに配車/visibility/acceptanceを載せる依頼がある場合だけ本skillも発火する。 | evidence dispositionのlocus/digest、必要ならdispatch overlay。 |
